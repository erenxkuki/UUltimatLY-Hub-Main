-- Load the library
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/erenxkuki/UUltimatLY-Hub/refs/heads/main/UUltimatLY_Hub_Lib.lua"))()

-- Create the window
local Window = Library:Make()

-- === CHANGE TO RED ACCENT ===
_G.Color = Color3.fromRGB(255, 50, 50)  -- Bright bold red (perfect for dark UI)

-- Global variable for your auto farm
_G.AutoFarmLevel = false
_G.Level = false  -- Map to toggle for the loop conditions

-- ===================================
-- Info Tab
-- ===================================
local InfoTab = Window:Tab("Info", 14103696428)  -- Optional icon

local InfoSection = InfoTab:CraftPage(1)  -- Left side (you can add right if needed later)

InfoSection:Seperator("Hub Info")

InfoSection:Label("Status: In Development")
InfoSection:Label("Version: Early Stage v0.1")
InfoSection:Label("Focus: Auto Farm Level Only")

InfoSection:Seperator("Player Actions")

InfoSection:Button("Rejoin Server", function()
    game:GetService("TeleportService"):Teleport(game.PlaceId, game.Players.LocalPlayer)
end)

InfoSection:Button("Server Hop", function()
    game:GetService("TeleportService"):Teleport(game.PlaceId, game.Players.LocalPlayer)
end)

-- ===================================
-- Auto Farm Tab (ONLY Auto Farm Level)
-- ===================================
local FarmTab = Window:Tab("Auto Farm", 14288624550)

local FarmSection = FarmTab:CraftPage(1)  -- Only left side needed for now

FarmSection:Seperator("Auto Farm Level")

FarmSection:Toggle("Auto Farm Level", false, function(state)
    _G.AutoFarmLevel = state
    _G.Level = state  -- Activate the loop conditions
    
    if state then
        print("[Hub] Auto Farm Level ENABLED - Start adding your code here!")
        --// Optimized BringEnemy Function (Compatible with Aurora Script)
        local plr = game.Players.LocalPlayer
        local TweenService = game:GetService("TweenService")
        local block = Instance.new("Part")  -- Define block here
        ...
        BringEnemy = function()
            local _B = true  -- Set _B to true when farming
        ...
        function BTP(p)
            ...
            repeat
                ...
                task.wait(0.5)
            until (p.Position - humanoidRootPart.Position).Magnitude <= 2000 or not _G.AutoFarmLevel
        end
        spawn(function()
            while task.wait() do
                pcall(function()
        ...
        function CheckQuest() 
            ...
            -- Simple world detection based on level (add placeId check if needed)
        ...
        print("[Hub] Auto Farm Level DISABLED")
        _G.Level = false  -- Stop conditions
        if workspace:FindFirstChild("TweenBlock") then
            workspace.TweenBlock:Destroy()  -- Clean up
        end
    end
end)

-- Optional load message
print("Minimal Hub Loaded! Only Auto Farm Level active. Press RightControl to toggle UI.")